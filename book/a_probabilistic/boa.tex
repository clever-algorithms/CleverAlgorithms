...
New solutions are sampled from the graph by first topologically ordering the graph (so that bits can be generated based on their dependencies), then probabilistically sampling the bits based on the conditional probabilities encoded in the graph. The algorithm for sampling the conditional probabilities from the network is called Forward Sampling, also referred to as Probabilistic Logic Sampling \cite{Henrion1988}.
