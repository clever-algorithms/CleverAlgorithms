...
\centering
\input{graphics/ga2}
\caption{Histogram of the best solution found by a Genetic Algorithm on a 300-bit OneMax problem over 100 runs.}
\label{plot:ga2}
\end{figure}
...
\caption{Heat map plot of the basin function showing the position of selected samples in the domain.}
\label{plot:pso1}
\end{figure}

\begin{lstlisting}[caption=Gnuplot script use to create a heat map and the position of selected samples from the domain., label=pso1]
set xrange [-5:5]
set yrange [-5:5]
set pm3d map
