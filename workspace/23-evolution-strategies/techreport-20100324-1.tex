...
\begin{itemize}
	...
	\item The ration of $\mu$ to $\lambda$ influences the amount of selection pressure (greediness) exerted by the algorithm.
	\item A contemporary update to the algorithms notation includes a $\rho$ as $(\mu/\rho,\lambda)-ES$ that specifies the number of parents that will contribute to each new candidate solution using a crossover operator. 
	\item A classical rule used to govern the amount of mutation (standard deviation used in mutation for continuous function optimization) was the $\frac{1}{5}$-rule, where a $\frac{1}{5}$ ratio of successful mutations should be 1/5 of all mutations. If it was greater the variance was increased, and if it was less it was decreased.
	\item The comma-selection variation of the algorithm can be good for moving optima and continued exploration, whereas the plus-selection variation can be good for refinement and convergence.
\end{itemize}
...
% algorithm
The algorithm is a implementation of Evolution Strategies based on simple version described by B\"ack and Schwefel \cite{Back1993b} and used in a detailed study \cite{Yao1997}.
%  cfg
...
% a good guide
More contemporary implements modify the strategy variables differently, and include an additional set of adapted strategy parameters to influence the direction of mutation (see \cite{Rudolph2000} for a concise description).
...
% Back
B\"ack, et al. provide a classical introduction to the technique, covering the history and development of the algorithm and the steps that lead it to where it was in 1991 \cite{Back1991}.
% Beyer and Schwefel
