...
\newcommand{\myreporttitle}{Artificial Immune Network}
...
The objective of the immune network process is to prepare a repertoire of discrete pattern detectors for a given problem domain, where better performing cells in the network suppress low-affinity (similar) cells in the network.
...
\begin{algorithm}[ht]
  ...
  % Algorithm
	...
	\Return{\Best}\;
	% end
	\caption{Pseudo Code for the Optimization Artificial Immune Network algorithm (opt-aiNet).}
	\label{alg:opt_ainet}
\end{algorithm}
...
\begin{itemize}
	...
	\item Mutation of clones is proportionate to the affinity of the parent cell with the cost function (better fitness, lower mutation).
	...
	\item The affinity threshold is problem and representation specific, for example a $AffinityThreshold$ may be set to an arbitrary value such as 0.1 on a continuous function domain, or calculated as a percentage of the problem space.
...
% the listing
\lstinputlisting[firstline=7,language=ruby,caption=Optimization Artificial Immune Network (opt-aiNet) in the Ruby Programming Language, label=optainet]{../../src/algorithms/immune/optainet.rb}
