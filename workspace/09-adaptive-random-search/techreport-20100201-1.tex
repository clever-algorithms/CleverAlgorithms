...
\begin{algorithm}[ht]
	...
	\SetKwData{InitialStepSizeFactor}{InitialStepSizeFactor}
	\SetKwData{SmallStepSizeFactor}{SmallStepSizeFactor}
	\SetKwData{LargeStepSizeFactor}{LargeStepSizeFactor}
	\SetKwData{LargeStepSizeFactorIter}{LargeStepSizeFactorIter}
	\SetKwData{MaxIterNoImprovement}{MaxIterNoImprovement}
	...
	\ForEach{$iter_i \in$ \NumIterations} {
		...
		$largesize_{step} \leftarrow 0$\;
		% bigger step
		\If{$iter_i \% $ \LargeStepSizeFactorIter} {
			$largesize_{step} \leftarrow$ $step_{size} \times$\LargeStepSizeFactor\;
		} 
		\Else {
			$largesize_{step} \leftarrow$ $step_{size} \times$\SmallStepSizeFactor\;
		}
		$candidate_2 \leftarrow$ \TakeStep{\SearchSpace \Current, $largesize_{step}$}\;
		
		% step size selection
		\If{\Cost{$candidate_1$}$<$\Cost{\Current} or \Cost{$candidate_2$}$<$\Cost{\Current}} {
			\If{\Cost{$candidate_2$}$<$\Cost{$candidate_1$}} {
				\Current $\leftarrow candidate_2$\; 
				$step_{size} \leftarrow largesize_{step}$\;
			}
			\Else {
		...
		\Else {
			$noimprov_{count} \leftarrow noimprov_{count} + 1$\;
			\If{$noimprov_{count} > $ \MaxIterNoImprovement} {
				$noimprov_{count} \leftarrow 0$\;
