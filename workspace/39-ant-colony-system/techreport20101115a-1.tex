...
where $\tau_{i,j}$ represents the pheromone for the component (graph edge) ($i,j$), $\rho$ is the decay factor, and $\Delta_{i,j}^{best} $ is the $\frac{1}{S_{cost}} $ (maximizing solution cost) for the best solution found so far. 
