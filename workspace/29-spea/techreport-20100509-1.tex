...
SPEA notes
- maintains an external population for each generation - sorts non-dominated solutions
- external population is mixed with the current population each generation
- fitness based on the number of solutions they dominate

- stores pareto optimal solutions externally
- uses pareto dominance to order and assign scalar fitness values
- uses clustering to reduce the number of non-dominated solutions will maintaining the structure of the Pareto front

SPEA2 notes
- external population is initially empty
- all non-dominated solutions from current pop are pushed to the external population
- two main differences: fitness assignment and truncation operator
- density is used as the fitness function
- spea1 uses a clustering algorithm when the non-dominated front exceeds the archive limit
	- use a new truncation operator
	- does not loose boundary points
- only members of the external population participate in mating
