...
% probem
The algorithm is applied to the Berlin52 instance of the Traveling Salesman Problem, taken from the TSPLIB. The problem seeks a permutation of the order to visit cities called a tour that minimized the total distance traveled. The optimal tour distance for Berlin52 instance is 7542.
% algorithm
todo... Berlin 52 symmetrical TSP, optima is , 2-opt local search

equation for setting lambda for TSP problems: $\lambda = \alpha\cdot\frac{cost(optima)}{N}$ where $N$ is the number of cities, and $cost(optima)$ is the cost of a local optimum found by a local search and $\alpha\in (0,1]$ (around 0.3 for TSP and 2-opt)

penalty utility function for TSP is: $U_{edge}=\frac{D_{edge}}{1+P_{edge}}$ where $U_{edge}$ is the utility for penalizing an edge (maximizing), $D_{edge}$ is the cost of the edge (distance between vectors) and $P_{edge}$ is the current penalty for the edge
