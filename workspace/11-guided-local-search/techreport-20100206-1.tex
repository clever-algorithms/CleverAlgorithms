...
minimizing optimization problem, local search uses an augmented cost function $h(s)=g(s)+\lambda\cdot\sum{i=1}{M}f_i$
...
\begin{algorithm}[htp]
	...
	\SetKwData{FeaturePenalties}{FeaturePenalties}
	% functions
	...
	\ForEach{$iter_i \in$ \NumIterations} {
		...
		% update
		$feature_{utilities} \leftarrow$ \CalculateFeatureUtilities{\Candidate, \FeaturePenalties}\;
		\FeaturePenalties $\leftarrow$ \UpdateFeaturePenalties{\Candidate, \FeaturePenalties, $feature_{utilities}$}\;
		% keep best
