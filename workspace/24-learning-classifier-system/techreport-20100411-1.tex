...
\begin{itemize}
	...
	\item The learning rate $\beta$ for a classifiers expected payoff, error and fitness could be in the range $\in [0.1,0.2]$.
	\item The frequency of running the genetic algorithm $\theta_{GA}$ should be in the range $\in [25,50]$.
	\item The discount factor used in multi-step programs $\gamma$ should be in the around $0.71$.
	\item The minimum error for whereby classifiers are considered to have equal accuracy $\epsilon_{0}$ should be 10\% of the maximum reward.
	\item The probability of crossover in the genetic algorithm $\chi$ should be in the range $\in [0.5,1.0]$.
	\item The probability of mutating a single position in a classifier in the genetic algorithm $\mu$ should be in the range $\in [0.01,0.05]$.
	\item The experience threshold during classifier deletion $\theta_{del}$ could be about 20.
	\item The experience threshold for a classifier during subsumption $\theta_{sub}$ could be around 20.
	\item The initial values for a classifiers expected payoff $p_1$, error $\epsilon_1$, and fitness $f_1$ should be small and close to zero.
	\item The probability of selecting a random action for the purposes of exploration $p_{exp}$ could be close to 0.5.
