...
Bacteria perceive their food and the direction to food based on the gradients of chemicals in their environment. Similarly, bacteria can perceive each other in a similar way secreting attracting and repelling chemicals into the environment. Using locomotion mechanisms (such as flagella) Bacteria can move around in their environment, sometimes moving chaotically (tumbling and spinning), and other times moving in a directed manner which one may refer to as swimming. Bacterial cells are treated like agents in an environment, using their perception of food and other cells as motivation to move, and stochastic tumbling and swimming like movement to re-locate. Depending on the cell-cell interactions, cells may swarm a food source, may aggressively repel or ignore each other.
...
\begin{align*}
	interaction(bacteria_k) = 
	&\sum_{i=1}^S\bigg[-d_{attract}\times exp\bigg(-w_{attract}\times \sum_{m=1}^P (bacteria_m^k - other_m^i)^2 \bigg) \bigg] + \\
	&\sum_{i=1}^S\bigg[h_{repellant}\times exp\bigg(-w_{repellant}\times \sum_{m=1}^P (bacteria_m^k - other_m^i)^2 \bigg) \bigg]
\end{align*}
