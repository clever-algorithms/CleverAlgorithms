...
\begin{itemize}
	...
	\item tabu search: sub-optimal moves (where hill climbing stops) are better starting points than random
	\item reason for RTS: straight tabu (fixed tabu) can be too slow in some situations
	\item reactive tabu is also about making tabu more efficient - using more efficient data structures (hash tables, etc)
	\item RTS provides a fully automated way of adapting the tabu list size to the problem
	\item also adds an escape strategy - the diversify the search when the tabu list is not enough
\end{itemize}

tabu search requires the a priori setting of the tabu list size, a size that defines the maximum length of cycles that can be avoided. cannot possibly know the optimal size, fixed may not be appropriate.
it is a tabu search that is augmented to detect the occurrence of cycles in the search and adapt the tabu list size accordingly
it reacts to the occurrence of repetitions

(automated tabu tenure)

also escaping mechanism seems to be core

strategy for reactive local search - automating the process by which a practioner configures an algorithm for a problem instance - use a feedback loop and automate it
