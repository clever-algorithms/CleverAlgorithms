...
\begin{description}
	\item[Keywords:] {\small\texttt{Clever, Algorithms, Description, Optimization, Genetic, Algorithm}}
\end{description} 
...
\begin{itemize}
	\item Binary strings (bitstrings) are the classical representation as they can be decoded to almost any desired representation.
	\item Continuous variables can be decoded using the binary coded decimal technique, although the gray code method may be more appropriate.
	...
	\item The schema theorem is a classical explanation for the power of the Genetic Algorithm proposed by Holland.
	\item The size of the population must be large enough to provide sufficient coverage of the domain and mixing of the useful sub-components of the solution. Goldberg, et al. provide a conservative estimation for the population size for binary strings as $\frac{1}{k}\times 2^k$ where $k$ is the size of sub-strings of interest, such as the number of bits for an encoded variable \cite{Goldberg1992}.
	\item The Genetic Algorithm is classically configured with a high probability of recombination (such as 95\%-99\% of the selected population) and a low probability of mutation (such as $\frac{1}{L}$ where $L$ is the number of components in a solution).
	\item The fitness-proportionate selection of candidate solutions to contribute to the next generation should be neither too greedy nor too random.
	\item The algorithm is highly-modular and a sub-field exists to study each component and problem specific operators are suggested. 
\end{itemize}
...
conservative: binary tournament selection for the selection operator, uniform crossover for the recombination operator, and point mutations for the mutation operator
