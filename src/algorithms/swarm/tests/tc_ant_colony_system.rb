# Unit tests for ant_colony_system.rb
...
require "test/unit"
require File.expand_path(File.dirname(__FILE__)) + "/../ant_colony_system"

class TC_AntColonySystem < Test::Unit::TestCase
  ...
  def test_initialise_pheromone_matrix
    rs = initialise_pheromone_matrix(100, 1.0)
    assert_equal(100, rs.size)
    rs.each do |x|
      assert_equal(100, x.size)
      x.each {|o| assert_equal(1, o)}
    end
  end
  ...
  # test updating global pheromone
  def test_global_update_pheromone
    pher = Array.new(5){|i| Array.new(5, 1.0)}
    global_update_pheromone(pher, {:vector=>[0,1,2,3,4],:cost=>2.0}, 1.0)
    edges = [[0,1],[1,2],[2,3],[3,4],[0,4]]
    pher.size.times do |x|
      pher.size.times do |y|
        if edges.include?([x,y]) or edges.include?([y,x])
          assert_equal(0.5, pher[x][y]) # replaced
        else
  ...
  # test updating local pheromone
  def test_local_update_pheromone
    pher = Array.new(5){|i| Array.new(5, 1.0)}
    local_update_pheromone(pher, {:vector=>[0,1,2,3,4]}, 0.5, 0.6)
    edges = [[0,1],[1,2],[2,3],[3,4],[0,4]]
    pher.size.times do |x|
      pher.size.times do |y|
        if edges.include?([x,y]) or edges.include?([y,x])
          assert_equal(0.8, pher[x][y]) # replaced
        else
          assert_equal(1.0, pher[x][y]) # no change
        end
      end
